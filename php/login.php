...
function logMessage($message) {
    $logFile = '/home/hillardcorp/public_html/logs/login.log';
    if (!file_exists(dirname($logFile))) mkdir(dirname($logFile), 0777, true);
    file_put_contents($logFile, date('[Y-m-d H:i:s] ') . $message . PHP_EOL, FILE_APPEND);
}

logMessage("Login request started");
...
if (!$email || !$password || !filter_var($email, FILTER_VALIDATE_EMAIL)) {
    logMessage("Invalid email or password: email=$email");
...
try {
    ...
    if ($agent) {
        if (!password_verify($password, $agent['password'])) {
            logMessage("Password mismatch for agent: email=$email");
            echo json_encode(['success' => false, 'message' => 'Invalid email or password']);
            exit;
        }
        if (!$agent['verified']) {
            logMessage("Unverified agent: email=$email");
        ...
        logMessage("Agent login successful: email=$email, redirect=$redirect");
    ...
    if ($defendant) {
        if (!password_verify($password, $defendant['password'])) {
            logMessage("Password mismatch for defendant: email=$email");
            echo json_encode(['success' => false, 'message' => 'Invalid email or password']);
            exit;
        }
        if (!$defendant['verified']) {
            logMessage("Unverified defendant: email=$email");
        ...
        logMessage("Defendant login successful: email=$email, redirect=/defendant-dashboard.html");
        echo json_encode(['success' => true, 'message' => 'Login successful', 'redirect' => '/defendant-dashboard.html']);
        exit;
    }

    logMessage("No user found: email=$email");
    echo json_encode(['success' => false, 'message' => 'Invalid email or password']);
} catch (Exception $e) {
